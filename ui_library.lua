...
local library = {design = "kali", tabs = {}, draggable = true, flags = {}, title = "Skylight", open = false, mousestate = inputService.MouseIconEnabled, popup = nil, instances = {}, connections = {}, options = {}, notifications = {}, tabSize = 0, theme = {}, foldername = "Skylight", fileext = ".cfg"}

local dragging, dragInput, dragStart, startPos, dragObject

local blacklistedKeys = {
	Enum.KeyCode.Unknown,Enum.KeyCode.W,Enum.KeyCode.A,Enum.KeyCode.S,Enum.KeyCode.D,Enum.KeyCode.Slash,Enum.KeyCode.Tab,Enum.KeyCode.Escape,Enum.KeyCode.Backspace
}
...
function library:LoadConfig(config)
	if table.find(self:GetConfigs(), config) then
		local Read, config_data = pcall(function() return cloneref(game:GetService("HttpService")):JSONDecode(readfile(self.foldername .. "\\Configs" .. "/" .. config .. self.fileext)) end)
		config_data = Read and config_data or {}
		for _, option in next, self.options do
			if option.hasInit then
				if option.type ~= "button" and option.flag and not option.skipflag then
					if option.type == "toggle" then
						spawn(function() option:SetState(config_data[option.flag] == 1) end)
					elseif option.type == "color" then
						if config_data[option.flag] then
							spawn(function() option:SetColor(config_data[option.flag]) end)
							if option.trans then
								spawn(function() option:SetTrans(config_data[option.flag .. " Transparency"]) end)
							end
						end
					elseif option.type == "bind" then
						spawn(function() option:SetKey(config_data[option.flag]) end)
					else
						spawn(function() option:SetValue(config_data[option.flag]) end)
					end
				end
			end
		end
...
function library:SaveConfig(config)
	...
	for _, option in next, self.options do
		if option.type ~= "button" and option.flag and not option.skipflag then
			if option.type == "toggle" then
				config_data[option.flag] = option.state and 1 or 0
			elseif option.type == "color" then
				config_data[option.flag] = {option.color.r, option.color.g, option.color.b}
				if option.trans then
					config_data[option.flag .. " Transparency"] = option.trans
				end
			elseif option.type == "bind" then
				if option.key ~= "none" then
					config_data[option.flag] = option.key
				end
			else
				config_data[option.flag] = option.value
			end
		end
	end
	writefile(self.foldername .. "\\Configs" .. "/" .. config .. self.fileext, cloneref(game:GetService("HttpService")):JSONEncode(config_data))
end

function library:GetConfigs()
	if not isfolder(self.foldername) or not isfolder(self.foldername .. "\\Configs") then
	...
	local files = {}
	local a = 0
	for i,v in next, listfiles(self.foldername .. "\\Configs") do
		if v:sub(#v - #self.fileext + 1, #v) == self.fileext then
			a = a + 1
			v = v:gsub(self.foldername .. "\\Configs" .. "\\", "")
...
library.createBind = function(option, parent)
	...
	library:AddConnection(inputService.InputBegan, function(input)
		...
		if binding then
			...
			option:SetKey(key or (not table.find(blacklistedKeys, input.KeyCode)) and input.KeyCode or "none")
		else
...
library.createList = function(option, parent)
	...
	option.listvalue = library:Create("TextLabel", {
		...
		BackgroundColor3 = Color3.fromRGB(50, 50, 50),
		BorderColor3 = Color3.new(),
		Text = "nil" .. (typeof(option.value) == "string" and option.value or getMultiText()),
		TextSize = 15,
		Font = Enum.Font.Code,
		TextColor3 = Color3.new(1, 1, 1),
		TextXAlignment = Enum.TextXAlignment.Left,
...
function library:Init()
	...
	self.base.Name = tostring(cloneref(game:GetService("HttpService")):GenerateGUID(false))

	self.main = self:Create("ImageButton", {
		AutoButtonColor = false,
		Position = UDim2.new(0, 250, 0, 300),
