...
local library = {design = "kali", tabs = {}, draggable = true, flags = {}, title = "Skylight", open = false, mousestate = inputService.MouseIconEnabled, popup = nil, instances = {}, connections = {}, options = {}, notifications = {}, tabSize = 0, theme = {}, foldername = "Skylight", fileext = ".config"}

library.title = "Skylight"

library.foldername = "Skylight"
...
function library:LoadConfig(config)
	if table.find(self:GetConfigs(), config) then
		local Read, config_data = pcall(function() return game:GetService"HttpService":JSONDecode(readfile(self.foldername .. "/" .. config .. self.fileext)) end)
...
function library:SaveConfig(config)
	local config_data = {}
	if table.find(self:GetConfigs(), config) then
		config_data = pcall(function() return game:GetService"HttpService":JSONDecode(readfile(self.foldername .. "/" .. config .. self.fileext)) end)
	end
	for _, option in next, self.options do
		if option.type ~= "button" and option.flag and not option.skipflag then
			if option.type == "toggle" then
	...
	writefile(self.foldername .. "/" .. config .. self.fileext, game:GetService"HttpService":JSONEncode(config_data))
end
