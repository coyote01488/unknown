...
function MacLib:Window(Settings)
			...
			function TabFunctions:InsertConfigSection(Side)
					...
					Callback = function()
						if not inputPath or string.gsub(inputPath, " ", "") == "" then
							WindowFunctions:Notify({
								Title = "Notification",
								Description = "Failed to create config. (Config name cannot be empty)"
							})
							return
						end
						...
						WindowFunctions:Notify({
							Title = "Notification",
							Description = string.format("Successfully created config %q.", inputPath),
						})
				...
				configSection:Button({
					...
					Callback = function()
						...
						WindowFunctions:Notify({
							Title = "Notification",
							Description = string.format("Successfully loaded config %q.", configSelection.Value),
						})
					end,
				})
				...
				configSection:Button({
					...
					Callback = function()
						...
						if not success then
							WindowFunctions:Notify({
								Title = "Notification",
								Description = "Failed to overwrite config."
							})
							return
						end
						...
						WindowFunctions:Notify({
							Title = "Notification",
							Description = string.format("Successfully overwrote config %q.", configSelection.Value),
						})
					end,
				})
				...
				configSection:Button({
					...
					Callback = function()
						...
						WindowFunctions:Notify({
							Title = "Notification",
							Description = string.format("Successfully set %q as autoload config.", name),
						})
					end,
				})
	...
	minimize.MouseButton1Click:Connect(function() end)
	exit.MouseButton1Click:Connect(function() end)
	...
	function MacLib:LoadAutoLoadConfig()
		...
		if isfile(MacLib.Folder .. "/settings/autoload.txt") then
			...
			if not suc then
				WindowFunctions:Notify({
					Title = "Notification",
					Description = "Failed to load autoload config."
				})
			end

			WindowFunctions:Notify({
				Title = "Notification",
				Description = string.format("Successfully loaded autoload config: %q.", name),
			})
		end
	end
