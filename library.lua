...
library.createSlider = function(option, parent)
	...
	option.title = library:Create("TextBox", {
		...
		BackgroundTransparency = 1,
		Text = (option.text == " " and "" or option.text .. ": ") .. string.format("%.2f", option.value) .. option.suffix,
	...
	option.title.Focused:connect(function()
		if not manualInput then
			...
			option.title.Text = (option.text == " " and "" or option.text .. ": ") .. option.value .. option.suffix
		end
	end)
...
library.createList = function(option, parent)
	...
	if option.sub then
		...
		option.main = library:Create("Frame", {
			LayoutOrder = option.position,
			Size = UDim2.new(1, 0, 0, option.text == " " and 30 or 48),
			BackgroundTransparency = 1,
			Parent = parent
		})
	...
	option.listvalue = library:Create("TextLabel", {
		Position = UDim2.new(0, 6, 0, (option.text == " " and not option.sub) and 4 or 22),
	...
	option.listvalue.InputBegan:connect(function(input)
		if input.UserInputType.Name == "MouseButton1" then
			...
			option.holder.Position = UDim2.new(0, pos.X + 6, 0, pos.Y + ((option.text == " " and not option.sub) and 66 or 84))
...
library.createBox = function(option, parent)
	...
	option.main = library:Create("Frame", {
		LayoutOrder = option.position,
		Size = UDim2.new(1, 0, 0, option.text == " " and 28 or 44),
		BackgroundTransparency = 1,
		Parent = parent
	})
	...
	option.holder = library:Create("Frame", {
		Position = UDim2.new(0, 6, 0, option.text == " " and 4 or 20),
		Size = UDim2.new(1, -12, 0, 20),
		BackgroundColor3 = Color3.fromRGB(50, 50, 50),
		BorderColor3 = Color3.new(),
		Parent = option.main
